...
export function buildUsageTooltip(durationMs: number, usage?: AgentEventUsage): string {
  ...
  if (usage) {
    ...
    if (pureInput > 0) lines.push(`输入: ${pureInput.toLocaleString()} token`)
    if (usage.outputTokens) lines.push(`输出: ${usage.outputTokens.toLocaleString()} token`)
    if (usage.cacheCreationTokens) lines.push(`缓存写入: ${usage.cacheCreationTokens.toLocaleString()} token`)
    if (usage.cacheReadTokens) lines.push(`缓存读取: ${usage.cacheReadTokens.toLocaleString()} token`)
  }
